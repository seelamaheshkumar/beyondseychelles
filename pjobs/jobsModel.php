...
class JobsModel {
    ...
    public function readAllJobs() {
        $sql = "SELECT * FROM tbljobs where order_status='Pending' order by jobid, orderdate desc";
    ...
    public function totalJobsRowCount() {
        $sql = "SELECT COUNT(*) as count FROM tbljobs where order_status='Pending'";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
    ...
    public function readJobsByDateRange($fdate, $tdate) {
       //$sql = "SELECT * FROM tbljobs where order_status='Pending' AND orderdate BETWEEN :fdate AND :tdate order by jobid desc";
       //$sql = "SELECT * FROM tbljobs where order_status='Pending' order by jobid desc";
       $sql = "SELECT * FROM tbljobs where order_status='Pending' order by jobid desc";
         //$sql="SELECT * FROM tbljobs WHERE order_status = 'Pending' AND YEAR(orderdate) = 2026 ORDER BY jobid DESC";
        $stmt = $this->conn->prepare($sql);
    ...
    public function totalJobsRowCountByDateRange($fdate, $tdate) {
        //$sql = "SELECT COUNT(*) as count FROM tbljobs where order_status='Pending' AND orderdate BETWEEN :fdate AND :tdate";
        $sql = "SELECT COUNT(*) as count FROM tbljobs where order_status='Pending' ORDER BY jobid desc";
        $stmt = $this->conn->prepare($sql);
        //$stmt->bindParam(':fdate', $fdate);
        //$stmt->bindParam(':tdate', $tdate);
        $stmt->execute();
    ...
    public function getJobDetailsFromDatabase($jobId) {
        ...
        try {
            ...
            $query = "UPDATE tbljobs SET netvalue=netvalue-:newnet, discount=discount+:discount, cash=cash+:ucash, card=card+:ucard, cheque=cheque+:ucheque, wallet=wallet+:uwallet, paid_amount=paid_amount+:upaid_amount, balance=:ubalance WHERE jobid=:jid";
     ...
     public function updatePay_BAK($jobData) {
        ...
        public function updateStatus($jobData) {
            try {
                ...
                $query = "UPDATE tbljobs SET order_status=:uorder_status, delivery_date=:delivery_date WHERE jobid=:jid";
                ...
                $stmtUpdateJob->bindParam(':delivery_date', $dt);
...
public function deleteJob_BAK($QId) {
    try {
      ...
      //  error_log($sqlDeleteQuotation);
        $stmtDeleteQuotation = $this->conn->prepare($sqlDeleteQuotation);
        // Bind parameter
        $stmtDeleteQuotation->bindParam(':id', $QId);
        // Execute the statement
        $stmtDeleteQuotation->execute();
       // error_log($stmtDeleteProducts->rowCount());
    //    error_log($stmtDeleteQuotation->rowCount());
