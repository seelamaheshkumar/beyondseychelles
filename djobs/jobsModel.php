...
class JobsModel {
    ...
    public function readAllJobs() {
        $sql = "SELECT * FROM tbljobs where order_status='Delivered' order by jobid, orderdate desc";
    ...
    public function totalJobsRowCount() {
        $sql = "SELECT COUNT(*) as count FROM tbljobs where order_status='Delivered'";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
    ...
    public function readJobsByDateRange($fdate, $tdate) {
        $sql = "SELECT * FROM tbljobs  where order_status='Delivered' AND delivery_date BETWEEN :fdate AND :tdate order by jobid,orderdate desc";
        $sql = "SELECT * 
        FROM tbljobs 
        WHERE order_status = 'Delivered' 
          AND DATE_FORMAT(delivery_date, '%Y-%m-%d') BETWEEN :fdate AND :tdate order by jobid, orderdate desc";
...
//        $sql = "SELECT COUNT(*) as count FROM tbljobs  where order_status='Delivered' AND delivery_date BETWEEN :fdate AND :tdate";
$sql = "SELECT COUNT(*) as count 
        FROM tbljobs 
        WHERE order_status = 'Delivered' 
          AND DATE_FORMAT(delivery_date, '%Y-%m-%d') BETWEEN :fdate AND :tdate";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':fdate', $fdate);
        $stmt->bindParam(':tdate', $tdate);
        $stmt->execute();
