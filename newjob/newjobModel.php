...
public function addJob($jobData) {
    try {
        ...
        $sqlInsertJob = "INSERT INTO tbljobs (qid, orderdate, company_name, contact_person, contact_no, address, oqty, ovalue, discount, vat, netvalue, credit_bill, cash, card, cheque, wallet, paid_amount, balance, createdby, jobno) 
                         VALUES (:qid, :orderdate, :company_name, :contact_person, :contact_no, :address, :tqty, :qvalue, :discount, :vat, :netvalue, :credit_bill, :cash, :card, :cheque, :wallet, :paid_amount, :balance, :createdby, :jobno)";
        ...
        $stmtInsertJob->bindParam(':jobno', $newJobno);
    ...
    public function addJob_v1($jobData) {
    try {
        if (!isset($jobData['company_name'], $jobData['contact_person'], $jobData['contact_no'], $jobData['address'], $jobData['tqty'], $jobData['qvalue'], $jobData['discount'], $jobData['vat'], $jobData['netvalue'], $jobData['credit_bill'], $jobData['cash'], $jobData['card'], $jobData['cheque'], $jobData['wallet'], $jobData['paid_amount'], $jobData['balance'])) {
            throw new InvalidArgumentException('Missing required job data.');
        }
